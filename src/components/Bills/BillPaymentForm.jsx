...
const BillPaymentForm = ({ open, onClose, onSuccess, bill }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      addNotification(
        `Bill payment of ₹${amount.toLocaleString()} for "${bill?.billName}" recorded from ${selectedAccount.accountName}`,
        'success',
        4000
      );
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle sx={{ fontWeight: 600, fontSize: '1.25rem' }}>
        Pay Bill - {bill?.billName}
      </DialogTitle>
      <DialogContent sx={{ pt: 3 }}>
        {error && <Alert severity="error" sx={{ mb: 2 }}>{error}</Alert>}

        <Box sx={{ mb: 2, p: 2, background: 'rgba(102, 126, 234, 0.1)', borderRadius: 2 }}>
        ...
        <TextField
          ...
          helperText={`Suggested: ₹${(bill?.amount || 0).toLocaleString()}`}
        />
